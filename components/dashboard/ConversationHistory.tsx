...
import { Clock3, Loader2, MessageSquarePlus } from 'lucide-react';
...
export default function ConversationHistory({ mode, activeSessionId, refreshKey, onSelect, onCreate }: ConversationHistoryProps) {
  ...
  const list = sessions.length ? sessions.map((session) => (
    <button
      key={session.id}
  ...
  )) : <p className="px-3 py-6 text-xs text-text-muted text-center">暂无历史对话</p>;
