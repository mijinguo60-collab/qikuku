...
export async function writeAuditLog(entry: AuditEntry) {
  try {
    ...
    await db.prepare(`INSERT INTO "AuditLog" (id, "companyId", "userId", action, "targetType", "targetId", result, ip, "createdAt") VALUES (?,?,?,?,?,?,?,?,?)`)
      .run(uuid(), entry.companyId || '', entry.userId || null, entry.action, entry.targetType || null, entry.targetId || null, entry.detail || null, entry.ip || null, new Date().toISOString());
