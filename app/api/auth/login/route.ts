...
export async function POST(request: NextRequest) {
  ...
  const result = await authenticateWithPhonePassword(phoneE164, input.data.password, metadata(request));
  if (!result.ok) {
  ...
  try {
    const session = await createServerSession(result.user, undefined, { rememberMe: input.data.rememberMe });
