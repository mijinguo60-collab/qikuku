  ...
  return (
    <div className="w-full max-w-md">
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <input className="input-primary -mt-2" type="password" autoComplete="current-password" maxLength={128} placeholder="请输入登录密码" value={password} onChange={(event) => setPassword(event.target.value)} />
      ...
      <p className="mt-6 text-center text-sm text-text-secondary">没有账号？ <Link className="font-medium text-accent-blue" href="/auth/register">注册企业</Link></p>
      <p className="mt-4 text-center text-xs text-text-muted">微信登录将在备案与微信资质完成后开放。</p>
    </div>
  );
}
